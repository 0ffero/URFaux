function rollDice() {
	...
	for (diceNum=0; diceNum<4; diceNum++) {
		...
		for (i=1; i<=randomNumber; i++) {
			// Dice are 4 sided, 2 of the roll are painted white, positions 1 and 2 are counted as a "dot", positions 3 and 4 are counted as "no dot"
	...
	fader = setInterval( function() {
		...
		if (j==5) {
			...
			// update movelist variable on page - this means we can catch a throw of 0 as the rest of it is filled by clicking on the counter
			moveListVar = $("#variables").data('movelist');
			if (roll==0) { endvar = ';' } else { endvar = ',' }
			if (moveListVar=="") { // ie is this the first throw? I could set a variable and check it but thats programming for the sake of it
				$("#variables").data('movelist', roll.toString() + endvar);
			} else {
				$("#variables").data('movelist', moveListVar + roll.toString() + endvar)
			}
...
function getPossibleMoves(roll) {
	moveList = "";
	$(".diceheader").html("You threw a " + roll);
	...
	if (roll>0) {
		console.log(roll + " moves. Getting possible moves");
		if (currentPlayer == 1) {
		...
		for (i=1; i<=7; i++) { // FOR EACH COUNTER
			...
			if (currentPosition != "FNA" && currentPosition != "FNB") {				
				...
				movepostemp = currentPositionID + roll;
		...
		if (moveList.length != 0) {
			...
			if (roll == 1) { plural = ""; } else { plural="s"; }
			popup("You threw a " + roll, "Click one of your counters to move it<br/>" + roll + " space" + plural, 0);
			setTimeout( function() {
				
			}, 2000);
		} else {
			console.log("No available moves. Moving on to next player");
			popup("No Available Moves", "You threw a " + roll + ". You have no available moves though.<br/>Moving on to other player.", 1);
